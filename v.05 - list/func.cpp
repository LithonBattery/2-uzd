...
float mediana (stud &tmp)
{
    ...
    if (tmp.paz.size()%2 == 0)
    {
        auto itr = tmp.paz.begin();
        for (int i=0; i<tmp.paz.size()/2; i++)
        {
            itr++;
        }
        t = (*itr + *--itr)/2.0;
...
void spausd (const list<stud> &v, bool ari, bool arvid, string filename)
{
    ...
    for (int i=0; i<v.size(); i++)
    {
        ...
        advance(ptr, i);
        stud tmp = *ptr;
        buff << setw(20) << left << tmp.vardas << setw(20) << left << tmp.pavarde << setw(20) << left << fixed << setprecision(2) << tmp.g << endl;
        if (i==1000000*tm)
        {
            if (ari==1)
            {
...
void atskyrimas (list<stud> &v, bool ari, bool arvid)
{
    ...
    list<stud> g(i, v.end()), tempas(v.begin(), i);
    //v.resize(v.size()-g.size());

    auto trp = std::chrono::high_resolution_clock::now();
    cout << "-------------------------------------" << endl;
    cout << "Studentai suskirstyti per " << std::chrono::duration_cast<std::chrono::microseconds>(trp-tr).count()/1000000.0 << " sec" << endl;
    cout << "-------------------------------------" << endl;

    string fg = "topai.txt", fb = "po_apacia.txt";
    ari = 1;
    ofstream out1 (fb, std::ios_base::trunc);
    out1.close();
    spausd (v, ari, arvid, fb);
    ofstream out2 (fg, std::ios_base::trunc);
    out2.close();
